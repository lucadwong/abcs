...
class ABCs():
    def __init__(self, env, exp_type=None, alpha = 0.05, max_nodes=np.inf, stat_level="action", boltzmann=False, temp=1, gamma=1.0, debug=False):
    ...
    def _reset(self):
        ...
        self.branch_percent = []
        self._branch_counts = 0
        self._no_branch_counts = 1
        self.stationary = defaultdict(bool)
        self._infoset_stat = 1
        self._infoset_nonstat = 1
        self.infoset_percents = []
        ...
        self._nonstat_counts = 1
        self._stat_counts = 0
        self._nonstat_states = defaultdict(int)
        self.percents = []

        # Nonstationary detection
        # self._true_states = defaultdict(list)
        self._true_states = defaultdict(lambda: collections.deque(maxlen=1000))
        self._state_dict = {}
        ...
        self._spiel_count = 0
        self._cartpole_stat_count = 0
        self._spiel_stat_count = 0
        self._cartpole_nonstat_count = 0
        self._spiel_nonstat_count = 0

    def _temp_schedule(self):
        '''Updates BQL temperature according to exponential scheme.'''
        if self._iteration % 20 == 0:
            self._temp = self._temp * 0.99
    ...
    def _get_action_probs(self, details, policy_type):
        ...
        if policy_type == "exploration":
            ...
            if all_visited:
                ...
                if self._boltzmann and self.check_infoset_stationary(details):
                    self._infoset_stat += 1
                    softmax_factor = self._temp
                else:
                    self._infoset_nonstat += 1
                    softmax_factor = 1/self._max_action[info_set]

                # softmax_factor = max(self._temp, 1/self._max_action[info_set])
    ...
    def _query_child(self, state, action, expand_child):
        ...
        # Action level stationarity detection
        if self._stat_level == "action":
            if done:
                detect_state = reward
            else:
                detect_state = self._env.get_detect_state(next_state)

            if str(detect_state) not in self._state_dict:
                self._state_dict[str(detect_state)] = self._state_counter
                self._state_counter += 1

            self._true_states[(info_set, action)].append((self._state_dict[str(detect_state)], self._iteration))
        ...
        if not done:
            ...
            if expand_child:
                ...
                continuation_value = reward + self.gamma * self._abcs_helper(next_player, next_state)
    ...
    def _core_update(self, state, child_data, cur_policy):
        ...
        for idx, action in enumerate(legal_actions):
            ...
            if not done:
                ...
                if not self._check_stationarity(player, info_set, action)[0] and not self.update_repeat[info_set, action]:
                    ...
                    assert nonstationary_correction[action] < 1000, "Nonstationary correction too large"
    ...
    def _abcs_helper(self, player, state):
        ...
        # Infoset level stationarity
        if self._stat_level == "info":
            detect_state = self._env.get_detect_state(state)
            if str(detect_state) not in self._state_dict:
                self._state_dict[str(detect_state)] = self._state_counter
                self._state_counter += 1

            self._true_states[info_set].append((self._state_dict[str(detect_state)], self._iteration))
        ...
        for idx, action in enumerate(legal_actions):
            ...
            stationary, defaulted = self._check_stationarity(player, info_set, action)
            ...
            if not defaulted:
                if stationary:
                    ...
                    try:
                        if len(info_set) == 4:
                            self._cartpole_stat_count += 1
                        else:
                            self._spiel_stat_count += 1
                    except:
                        pass
                    ...
                    try:
                        if len(info_set) == 4:
                            self._cartpole_nonstat_count += 1
                        else:
                            self._spiel_nonstat_count += 1
                    except:
                        pass

            if not stationary and not self.expand_repeat[info_set, action]:
                self._branch_counts += 1
            else:
                self._no_branch_counts += 1

            # First condition is BQL branching. Second one is force branching stationary for debugging. Last one is if we think you're nonstationary.
            # However, we only ever branch the FIRST instance we see an infostate (if there are repeats)
            expand_child = (action == sampled_action or (not self._check_stationarity(player, info_set, action)[0] and not self.expand_repeat[info_set, action]))
            if expand_child:
                child_data[idx] = self._query_child(state, action, expand_child=expand_child)
            else:
                child_data[idx] = self._query_child(state, action, expand_child=False)
        ...
        # Note that we expand the children BEFORE doing the update. We should try to test this actually?
        self._core_update(state, child_data, cur_policy)
    ...
    def _check_stationarity(self, player, info_set, action):

        if (info_set, action) in self._cache:
            return self._cache[(info_set, action)]

        if self._stat_level == "action":
            state_dist = list(self._true_states[(info_set, action)])
        else:
            state_dist = list(self._true_states[info_set])

        if len(state_dist) < 10:
            self._cache[(info_set, action)] = True, True
            return True, True
        else:
            first_half, second_half  = [x for (x, y) in state_dist[:len(state_dist) // 2]], [x for (x, y) in state_dist[len(state_dist) // 2:]]
            first_counts, second_counts = collections.Counter(first_half), collections.Counter(second_half)
            all_states = set(first_counts.keys()).union(set(second_counts.keys()))
            
            first_totals = []
            second_totals = []
            for state in all_states:
                if state in first_counts:
                    first_totals.append(first_counts[state])
                else:
                    first_totals.append(0)

                if state in second_counts:
                    second_totals.append(second_counts[state])
                else:
                    second_totals.append(0)

            pval = scipy.stats.chi2_contingency([first_totals, second_totals])[1]

            # print(info_set, first_totals, second_totals, pval)
            self._cache[(info_set, action)] = (pval > self._alpha), False

            return (pval > self._alpha), False

    def check_infoset_stationary(self, details):
        """Checks if infoset is stationary (infoset is non-stationary if and only if any action is non-stationary)"""
        player, info_set, legal_actions = details

        for action in legal_actions:
            if not self._check_stationarity(player, info_set, action)[0]:
                return False
    ...
    def run_round(self):
        """Runs a single iteration of ABCs."""

        if self._iteration % 10000 == 0:
            print(self._nonstat_counts, self._stat_counts)
            print(self._temp)
        ...
        # Add statistics about current iteration
        self.percents.append(self._nonstat_counts / (self._nonstat_counts + self._stat_counts))
        self.branch_percent.append(self._branch_counts / (self._branch_counts + self._no_branch_counts))
        self.nodes_touched_by_iteration.append(self._nodes_touched)
        self.infoset_percents.append(self._infoset_nonstat / (self._infoset_stat + self._infoset_nonstat))
